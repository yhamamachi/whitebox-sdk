...
DOMD_NEW_FILE="        - \"tmp/work/s4sk_proto-poky-linux/rcar-image-minimal/1.0-r0/rootfs/var/statestorage.db\""
...
DOMD_VAR_NEW_FILE="          \"statestorage.db\": \"%{YOCTOS_WORK_DIR}/build-domd/tmp/work/s4sk_proto-poky-linux/rcar-image-minimal/1.0-r0/rootfs/var/statestorage.db\""
OLD_COMMIT=d1e67027b82143bd56cc8a881e11e4d475dcffb9
NEW_COMMIT=e74fea0be1d2553617f13344babdfbeb0cee4eeb
S4SK_PATCH_PATH="${SCRIPT_DIR}/s4sk_patch/*"
...
# Update yaml file
sed -i "s/${DOMD_OLD_SIZE}/${DOMD_NEW_SIZE}/" aos-rcar-gen4-s4sk.yaml
sed -i "s|\(\".unprovisioned\":.*\"\)|\1\n${DOMD_VAR_NEW_FILE}|" aos-rcar-gen4-s4sk.yaml
sed -i "s|\(  domu:$\)|${DOMD_NEW_FILE}\n\1|" aos-rcar-gen4-s4sk.yaml
sed -i "0,/\"..\/meta-aos\"/s|\(\"../meta-aos\"\)|\1\n${DOM0_BITBAKE_NEW_LAYER}|" aos-rcar-gen4-s4sk.yaml
sed -i "s|\(\"../meta-aos-rcar-gen4/meta-aos-rcar-gen4-driver-domain\"\)|\1\n${DOMD_BITBAKE_NEW_LAYER}|" aos-rcar-gen4-s4sk.yaml
sed -i "s|\(        - \[XT_DOM_NAME, \"domd\"\]$\)|\1\n${IMAGE_INSTALL_DOMD}|" aos-rcar-gen4-s4sk.yaml
sed -i "s|\(        - \[XT_DOM_NAME, \"domu\"\]$\)|\1\n${IMAGE_INSTALL_DOMU}|" aos-rcar-gen4-s4sk.yaml
...
# Apply S4SK patch
git -C ./yocto/meta-aos-rcar-gen4 am ${S4SK_PATCH_PATH}
...
moulin ../aos-rcar-gen4-s4sk.yaml
ninja
ninja image-full
echo "Build finished !"
